 ...
 * iosys_retrocade.v
 *
 * FPGA Companion SPI-based IO system for nestang on Papilio Retrocade.
 * Replaces iosys_bl616 (UART-based) with an SPI-based interface that
 * communicates with the ESP32 FPGA Companion MCU.
 *
 * Adapted from snestang/src/papilioarcade/iosys_retrocade.v.
 * Key difference from SNES version: CORE_ID defaults to 1 (nestang).
 * The rom_do/rom_do_valid byte stream feeds nestang's game_loader.v which
 * parses the iNES header and writes parsed PRG/CHR ROM to SDRAM.
 *
 ...
 *
 * NES button format [11:0] (SNES layout, lower 8 bits = NES):
 *   bit7=RIGHT, bit6=LEFT, bit5=DOWN, bit4=UP,
...
module iosys_retrocade #(
    ...
    // Overlay interface to nes2hdmi (same timing as textdisp: 2-cycle latency on hclk)
    output reg       overlay,
    input  [7:0]     overlay_x,
    input  [7:0]     overlay_y,
    output reg [14:0] overlay_color,

    // HID joystick output (12-bit SNES layout, lower 8 bits = NES buttons)
    ...
    // ROM loading interface -> game_loader.v
    ...
    hid hid_inst (
        ...
        .joystick1ay    (),
        .joystick_strobe(),
        // A2600-specific sysctrl inputs (not used for NES)
        .p_dif1         (1'b0),
        .p_dif2         (1'b0),
        .p_color        (1'b0)
    );
    ...
    // ESP32 firmware (sysctrl.c sys_handle_interrupts) dispatches:
    //   bit 0 -> sys_handle_event   (SYSCTRL, not used here)
    //   bit 1 -> hid_handle_event   (HID)
    //   bit 3 -> sdc_handle_event   (SDC)
    // Bit positions MUST match — otherwise SDC IRQs get routed to HID handler
    // and ROM loading never completes.
    assign int_in   = {4'b0000, sdc_irq, 1'b0, hid_irq, 1'b0};
    assign hid_iack = int_ack[1];
    assign sdc_iack = int_ack[3];
    ...
    // =========================================================================
    // ROM Loader FSM: load ROM from SD card to game_loader.v byte stream
    //
    // Flow:
    //   1. MCU sends SDC CMD 4 (INSERTED) -> image_size captured, sd_image_mounted[0] pulses
    //   2. FSM asserts rstart[0] for sector 0 -> sd_card IRQ fires
    //   3. MCU translates logical sector -> physical LBA via FAT cluster table, sends CMD 2
    //   4. FPGA reads physical sector from SD card
    //   5. sd_outen bytes forwarded to rom_do/rom_do_valid -> game_loader.v
    //   6. game_loader parses iNES header and writes PRG/CHR ROM to SDRAM
    //   7. After image_size bytes, rom_loading deasserted
    // =========================================================================
    reg [2:0]  ld_state;
    ...
    always @(posedge clk) begin
        if (~resetn) begin
            ...
            LD_READING: begin
                ...
                // Forward each output byte from SD card to ROM loader (game_loader.v)
                if (sd_outen) begin
    ...
    // =========================================================================
    // Joystick mapping: FPGA Companion HID -> 12-bit SNES/NES button format
    //
    // SNES/NES [11:0]: bit11=X, bit10=A, bit9=RT, bit8=LT, bit7=R, bit6=L,
    //                  bit5=DOWN, bit4=UP, bit3=START, bit2=SELECT, bit1=Y/B, bit0=B/A
    // NES uses lower 8 bits: bit7=RIGHT, bit6=LEFT, bit5=DOWN, bit4=UP,
    //                         bit3=START, bit2=SELECT, bit1=B, bit0=A
    //
    ...
    always @(posedge clk) begin
        hid1 <= {
            ...
            joystick0[4],       // bit10: A  (Cross)   — NES A
            ...
            joystick0[7],       // bit1:  Y/B (Triangle) — NES B
            joystick0[5]        // bit0:  B/A (Circle)   — NES A alt
        };
        hid2 <= {
            joystick1[6],
            joystick1[4],
    ...
    wire [6:0]  ox_comb     = overlay_x - 8'd64;
    wire [5:0]  oy_comb     = overlay_y - 8'd80;
