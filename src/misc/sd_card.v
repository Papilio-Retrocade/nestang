         ...
         if(data_start) begin
			...
			if(command == 8'd4) begin
			   ...
			   if(byte_cnt == 4'd4) begin 
				 ...
				 // Only fire image_mounted for a real insertion (size > 0).
				 // sdc_image_open sends eject (size=0) before insert; firing
				 // on the eject would deadlock the ROM loader FSM because the
				 // file handle isn't open yet when the FPGA requests sector 0.
				 if({ image_size[63:8], data_in } != 64'd0)
